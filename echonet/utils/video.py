...
@click.option("--task", type=str, default="EF")   #LargeTrace  EF
    ...
    with open(os.path.join(output, "log.csv"), "a") as f:
        ...
        if run_test:
            for split in ["val", "test"]:
                ...
                with open(os.path.join(output, "{}_EF.csv".format(split)), "w") as g:
                    g.write("Filename,yhat,y\n")
                    for (filename, y_hat,yy) in zip(dataset.fnames, yhat, y):  # zip用于将多个迭代器对象打包成一个元组序列
                        g.write("{},{},{}\n".format(filename, y_hat,yy ))
                ...
                # Plot actual and predicted EF  绘图
